...
class DemandeRdvPage extends StatefulWidget {
  @override
  _DemandeRdvPageState createState() => _DemandeRdvPageState();
}

class _DemandeRdvPageState extends State<DemandeRdvPage> {
  int? userId;
  String? nom;
  String? prenom;
  String? email;
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      // Récupérer les données de véhicule retrouvé sauvegardées
      String? retrouveDataJson = prefs.getString('retrouveData');
      if (retrouveDataJson == null) {
        _showErrorDialog('Données du véhicule non trouvées');
        return;
      }
      final retrouveData = jsonDecode(retrouveDataJson);

      // Combine les données du véhicule retrouvé et de la demande de rendez-vous
      final allData = {
        ...retrouveData, // Données du véhicule retrouvé
        'motifData': motifData, // Données de la demande de rendez-vous
      };

      try {
        final response = await http.post(
          Uri.parse('$url/api/vehiculeRetrouveAvecRdv'),
          headers: {
